...
- [x] Offline uygulamanın sol üst marka alanında koyu logo yerine açık renkli mühür tarzı Global 1881 işaretini kullan
...
- [x] Sözleşme sahibi dışındaki danışmanların Yetki/Kira A4 önizlemesini ve yazdırma yolunu teknik olarak engelle; broker ve ofis asistanı için yetkili görüntülemeyi koru
- [x] Yetkisiz danışman listelerinde malik adı, kimlik/VKN, telefon ve adresi geri döndürülemez maskeli özet olarak göster; tam müşteri/sözleşme snapshot’ını istemciye açma
- [x] Rol ve sahiplik kurallarını Yetki, Kira, Benim Sözleşmelerim, kayıt listeleri ve yazdırma akışlarında birim/UI testleriyle doğrula
...
- [x] Offline sol menüde Global 1881 marka alanını daha büyük mühür/logo ve güçlü tipografiyle; bölüm gruplarını, aktif/pasif menü hiyerarşisini ve alt kullanıcı alanını sağ Ofis Akışı paneliyle dengeli güçlü bir Windows görsel diline taşı
- [x] Mevcut kabul kümesinde offline sol menünün Global 1881 marka alanı, bölüm başlıkları, aktif/pasif gezinme durumu ve dar ekran erişilebilirliğini; sağdaki tek Size Özel Gündem panelini çoğaltmadan ortak görsel hiyerarşide güçlendir
- [x] Ortak online/offline sol menünün kullanıcı tarafından aşırı genişletilmesini engelle: varsayılan 280 px genişliği koru, üst yeniden boyutlandırma sınırını ana form ve sağ yardımcı panel dengesini bozmayacak ölçüye indir; dar ekran Drawer davranışını değiştirme
...
- [x] Yetki, kira, yerel çalışma alanı ve işlem kapanışları ana form yüzeylerini bölümlenmiş kartlar, daha belirgin alan başlıkları, okunur giriş çerçeveleri ve kontrollü kontrastla ortak standartta düzenle; sağ Size Özel Gündem panelini yardımcı sütun olarak daraltıp ana formun önüne geçirmeme kuralını uygula
- [x] Mevcut kabul kümesinde Yetki, Kira, Yerel Çalışma Alanı ve İşlem Kapanışları ekranlarının ortak `offline-operation` grid, bölüm kartı, form alanı ve sağ yardımcı panel uygulamalarını envanterle; küçük uyumsuzlukları yeni Windows paketi üretmeden biriktirerek ortak standartta düzelt
...
- [x] İşlem Kapanışları tahsilat beyanı ve opsiyonel kapora formunda kalan `input type="date"` alanlarını da ortak Türkiye biçimli tarih girişine taşı; ISO tarih saklama, tahsilat/audit verisi ve işlem kapanışı hesaplarını değiştirme
...
- [x] Online ve offline DashboardLayout, sol menü, içerik çerçevesi, form/kart, düğme, seçici ve sağ gündem paneli arasındaki renk ve ölçü farklarını envanterle
- [x] Mevcut kabul kümesinde online ve offline DashboardLayout’un ortak marka, aktif/pasif menü, içerik çerçevesi, kart ve form denetim sınıflarını karşılaştır; kullanıcı akışını veya Electron güvenlik sınırını değiştirmeden ölçü/tipografi farklarını tek görsel dilde hizala
...
- [ ] Merkezi aktarım uygulamasında manager dışı tüm kullanıcıları engelle; yedek doğrulama/fark önizlemesi ayrı çalışsın, apply isteğinde tam teyit metni ve önizleme kimliği zorunlu olsun, her satır için kaynak kimliği/audit ve geri alma manifesti üret, offline PDF baytlarını otomatik yükleme
- [x] Kullanıcı onayıyla merkezi toplu aktarım yerine `Geçiş Tarihi ile Temiz Online Başlangıç` modelini uygula: manager yalnız geçiş tarihini ayarlasın; açılış bakiyesi, geçmiş kasa/banka, devir veya eski finansal hareket taşınmasın; bu tarihten sonra yeni merkezi kayıtlar tek kaynak olsun, eski offline sözleşme/PDF/arşiv ve yedekler taşınmasın veya silinmesin; yalnız devam eden aktif dosyalar özet başlangıç kaydıyla manuel açılabilsin, tüm kararlar auditlensin
- [x] Temiz online başlangıç çekirdeğini uygula: yalnız broker manager tam teyit metniyle geçiş tarihini ayarlasın; ayar audit kaydına yazılsın, açılış bakiyesi/devir/aktarım alanı sunulmasın, yeni merkezi müşteri-portföy-sözleşme-vade-ledger-kasa hareketleri bu tarihten önce oluşturulamasın
- [x] Merkezi başlangıç ayarlanmamış veya ileri tarihliyken web ve mobil kullanıcılara salt-okunur durum bildirimi göster; danışmanların eski veri aktarımı olduğu izlenimine kapılmasını veya başarısız yeni kayıt denemesini önle, rol gizliliği ve mevcut verileri değiştirme
...
- [x] Dar Electron görünümünde DashboardLayout’un SidebarTrigger menüsü varken App içindeki yinelenen `md:hidden` offline hızlı rota şeridini kaldır; hash rotaları ve erişilebilir mobil/dar ekran gezinmesini tek sol menü kaynağında koru
...
- [x] Web ana sayfasını ve sık sözleşme girişlerini anında tutarken online ikincil yönetim ekranlarını tembel yüklemeye taşı; Wouter rotalarını aynı bırak, ortak yüklenme yüzeyini kullan, üretim başlangıç paketi boyutunu ölç ve yeni ZIP üretme
- [x] Mobil danışman uygulamasını yalnız `/mobile` rotasında tembel yükle; ana web/Windows kabuğuna yük bindirme, aynı RouteLoader yüzeyini kullan, mobil rota erişimini ve üretim paketi boyutunu test et, yeni ZIP üretme
- [x] PnPM 10’un package.json içindeki artık okunmayan `pnpm` ayarları uyarısını gider: Wouter patch ve Tailwind nanoid override’ını desteklenen `pnpm-workspace.yaml` yapılandırmasına taşı, kilitli/yerel kurulumun uyarısız çalıştığını doğrula, kullanıcı verisine ve Windows kurulum akışına dokunma
...
- [x] EİDS Yetki Numarası alanını Malik adı/unvanı ve TCKN/VKN satırının hemen altında, Malik telefonu alanının üstünde, çağrılmış kayıt seçimine bağlı olmayan tam genişlikte sabit bir form satırına taşı; yalnız-rakam girişi, bekliyor/tamamlandı durumu ve müşteri A4 gizliliğini testle doğrula
- [ ] Windows 1.0.20’de kullanıcının göremediği `EİDS Yetki Numarası` alanını Malik adı/unvanı ve TCKN/VKN satırının hemen altında, Malık telefonu alanının üstünde, çağrılmış kayıt koşulundan bağımsız ve formda sürekli görünür hâle getir; kaynak/Windows ekran kanıtıyla doğrula
...
- [x] Bütçe ve Gider Kontrolü modülünde personel giderlerini ayrı yönetim grubu olarak göster; temsil/ağırlama giderlerinde broker managerin ayarlayabileceği tutar eşiği aşılınca bilgi kartı ve onay kaydı oluştur, eşik altındaki kaydı normal bütçe gerçekleşeni olarak işle
- [x] Bütçe ve Gider Kontrolü modülünde temsil/ağırlama broker bilgi/onay eşiğini başlangıçta `3.000 TL` yap; yalnız broker managerın değiştirebildiği ayar olarak sakla ve `3.000 TL` ile üzerindeki kayıtlarda bekleyen onay durumunu uygula
- [x] Bütçe aşımında yalnız broker managerın fasıllar arası bütçe aktarımı yapabilmesini sağla; kaynak/hedef fasıl, tutar, gerekçe, tarih ve onaylayan kişiyi değişmez iz olarak sakla, kaynak fasıl kalan bütçesini aşan aktarımı engelle ve raporda ilk bütçe/aktarım/revize bütçe ayrımını göster
- [x] Resmî muhasebe yerine geçmeyen `Ofis Payı ve Danışman Katkı Kontrolü` katmanını tasarla: sözleşme/işlemden gelen hizmet bedeli, danışman payı, ofis payı, sistem içi tahsilat ve sistem dışı manuel nakit bildirimi ayrı izlenmeli; broker manager aylık/yıllık tabloda danışman başına kişisel üretim, ofise katkı, bildirilen/tahsil edilen farkı ve eksik riskini görmelidir
- [x] Ofis Payı ve Danışman Katkı Kontrolü için varsayılan paylaşımı `%60 danışman / %40 ofis` uygula; danışman bazlı istisna ve işlem özelindeki oran değişikliği yalnız broker manager tarafından gerekçeli/izli yapılabilsin
...
- [x] Komisyon tahsilatından ofis kasasına yapılan isteğe bağlı `Ofis Kasasına Aktarılan Komisyon Payı` kaydını ekle: kaynak sözleşme/işleme, kasa hesabına, tutara, tarihe ve teslim alan yetkiliye bağla; ofis payı açık bakiyesini düşürsün fakat yeni tahsilat/hizmet bedeli üretmesin, kullanılmadığında hesaplamaları etkilemesin
- [x] Bütçe, ofis payı ve danışman katkı modülünü resmiyet taşımayan iç denetim/performans aracı olarak sınırla: e-Fatura kesimi, gelir beyanı, geçici vergi, yıl sonu mahsuplaşması, resmî muhasebe fişi ve beyanname üretme; mali müşavirden gelen toplamlar yalnız teyitli referans olarak isteğe bağlı girilebilsin
- [x] İç denetim kayıtlarını EİDS benzeri manuel tamamlama ilkesiyle tasarla: yetkili kişi resmî süreçten bağımsız ofis içi tutar/durum/referansı elle girsin; kayıt tarih-kullanıcı iziyle kalsın, ofisin bilgi kontrolünü güçlendirsin fakat resmî muhasebe/e-Fatura/vergi kaydını üretmesin veya değiştirmesin
- [x] İç denetim ekranında seçilebilir kapsam filtresi uygula: `Danışman ↔ Ofis Pay Hareketleri`, `Ofis Gelirleri`, `Personel Giderleri`, `Ofis Giderleri` ve `Tümü` seçenekleri tablo/özet hesaplamasına ayrı ayrı dahil edilebilsin veya çıkarılabilsin; danışman yalnız kendi pay hareketlerini görebilsin
- [x] Kullanıcı onayıyla Bütçe, Ofis Payı ve Danışman Katkı Kontrolü modülünü EİDS/A4 toplu kabulünden sonra bütün olarak uygula; %60/%40 KDV hariç paylaşım, 3.000 TL temsil/ağırlama eşiği, personel grubu, fasıl aktarımı, isteğe bağlı komisyon kasa ek girişi, manuel iç denetim kaydı ve seçilebilir kapsam filtrelerini tek veri modelinde/tek kabul paketinde birleştir
- [ ] 1.0.21 toplu Windows kabulünde EİDS alanının Malik kimlik satırının altında sürekli görünmesini, müşteri A4’ünde görünmemesini; Bütçe/Gider, Ofis Payı/Katkı, kişisel katkı özeti, 3.000 TL onayı ve kasa aktarımı akışlarını tek kısa kontrol setinde doğrula
- [ ] Kullanıcı talimatı doğrultusunda acil hata veya anlamlı toplu kabul sınırı olmadan yeni Windows sürümü/kaynak ZIP üretme; gelen küçük düzeltmeleri mevcut kabul listesinde biriktir
- [ ] Kullanıcının açık çalışma tercihi: hiçbir küçük/orta düzeltme sonrasında otomatik sürüm, kaynak ZIP veya Windows kurulum paketi üretme ve kullanıcıdan kurulum isteme; yalnız acil/bloklayıcı hata ya da kullanıcıyla önceden kararlaştırılmış anlamlı toplu kabul noktasında paketleme kararını ayrıca teyit et
- [x] Kullanıcı onayıyla `/home/ubuntu/Downloads` içindeki yalnız eski `Global1881-Ofis-Offline-*-Windows-Kaynak.zip` paketlerini temizle; tarih/sürüm bakımından en yeni iki ZIP’i koru, kullanıcı verisi, IndexedDB, imzalı PDF, kaynak kod, aktif kurulum ve diğer dosyalara dokunma
- [ ] Hızlandırılmış günlük kullanıma geçiş planını uygula: yeni ikincil modülleri dondur, EİDS/A4/gizlilik/iç denetim toplu kabulünü tek Windows oturumunda tamamla, manager onaylı merkezi aktarım önizleme–apply akışına odaklan ve DNS/özel alan adı ile gerçek cihaz kabulünü ayrı kısa aşamada bitir
- [x] Temiz online başlangıç çekirdeğini uygula: yalnız broker manager tam teyit metniyle geçiş tarihini ayarlasın; ayar audit kaydına yazılsın, açılış bakiyesi/devir/aktarım alanı sunulmasın, yeni merkezi müşteri-portföy-sözleşme-vade-ledger-kasa hareketleri bu tarihten önce oluşturulamasın
- [x] Merkezi başlangıç ayarlanmamış veya ileri tarihliyken web ve mobil kullanıcılara salt-okunur durum bildirimi göster; danışmanların eski veri aktarımı olduğu izlenimine kapılmasını veya başarısız yeni kayıt denemesini önle, rol gizliliği ve mevcut verileri değiştirme
- [x] Temiz başlangıç geçiş tarihini Europe/Istanbul iş günü başlangıcına normalleştir; eski offline PDF arşiv aktarımını merkezi yükleme uçlarında kapat, yalnız geçişten sonra oluşturulan merkezi sözleşmelere bağlı yeni imzalı PDF’lere izin ver
...
- [x] Kullanıcı talebiyle güncel menü düzenini içeren temiz kaynak ZIP’i oluştur, kullanıcı verilerini ve build artıkları dışarıda bırak, bütünlüğünü doğrula ve paylaş
- [ ] Tekrar eden ve eski Windows kabul kayıtlarını aktif işlerden ayır; geçmişi koruyan arşiv bölümü oluştur ve gerçek bekleyen görevleri sadeleştir
