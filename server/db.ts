...
import {
  ...
  parameterizeLandShareClauseBody,
...
export async function createClient(input: {
  ...
  const duplicate = (await db
    ...
    .limit(1))[0];
  if (duplicate) {
    throw new Error(`DUPLICATE_CLIENT_ID:${duplicate.id}|Bu müşteri merkezi kayıtlarda zaten mevcut: ${duplicate.referenceNo} · ${duplicate.name}${duplicate.consultantCode ? ` · Sorumlu danışman: ${duplicate.consultantCode}` : ""}. Yeni kayıt açmak yerine mevcut müşteri kaydını kullanın.`);
  }
...
export async function updateClient(input: {
  ...
  if (patch.name) {
    ...
    const duplicate = (await db
      ...
      .limit(1))[0];
    if (duplicate) throw new Error(`DUPLICATE_CLIENT_ID:${duplicate.id}|Bu isimle merkezi müşteri kaydı zaten mevcut: ${duplicate.referenceNo} · ${duplicate.name}${duplicate.consultantCode ? ` · Sorumlu danışman: ${duplicate.consultantCode}` : " · Sorumlu danışman bilgisi kayıtlı değil"}.`);
  }
...
export async function createCentralCommissionTransaction(input: {
  ...
  externalOfficeAgreementReference?: string;
  externalOfficeAgreementPartyName?: string;
  externalOfficeAgreementScopeNote?: string;
  externalOfficeAgreementMinFee?: string;
  externalOfficeAgreementMaxFee?: string;
  externalOfficeAgreementSignedAt?: Date;
  externalOfficeAgreementValidFrom?: Date;
  externalOfficeAgreementValidTo?: Date;
  ...
  const agreementMinFee = input.externalOfficeAgreementMinFee ? Number(input.externalOfficeAgreementMinFee) : null;
  const agreementMaxFee = input.externalOfficeAgreementMaxFee ? Number(input.externalOfficeAgreementMaxFee) : null;
  if (externalOfficeRate > 0 && (!input.externalOfficeAgreementReference?.trim() || !input.externalOfficeAgreementPartyName?.trim() || !input.externalOfficeAgreementScopeNote?.trim() || !input.externalOfficeAgreementSignedAt || !input.externalOfficeAgreementValidFrom || !input.externalOfficeAgreementValidTo)) throw new Error("Dış ofis paylaşımında anlaşma referansı, taraf/ofis adı, kapsam notu ve tarih alanları zorunludur.");
  if (externalOfficeRate > 0 && ((agreementMinFee !== null && (!Number.isFinite(agreementMinFee) || agreementMinFee < 0)) || (agreementMaxFee !== null && (!Number.isFinite(agreementMaxFee) || agreementMaxFee < 0)) || (agreementMinFee !== null && agreementMaxFee !== null && agreementMaxFee < agreementMinFee))) throw new Error("Anlaşma işlem büyüklüğü sınırları geçerli ve min tutar max tutarı aşmayacak şekilde girilmelidir.");
  if (externalOfficeRate > 0 && agreementMinFee !== null && grossNetServiceFee < agreementMinFee) throw new Error("İşlem bedeli dış ofis anlaşmasının asgari kapsam tutarının altında.");
  if (externalOfficeRate > 0 && agreementMaxFee !== null && grossNetServiceFee > agreementMaxFee) throw new Error("İşlem bedeli dış ofis anlaşmasının azami kapsam tutarını aşıyor.");
  ...
  await db.insert(auditLogs).values({ actorUserId, action: "commission-declared", entityType: "commissionTransaction", entityId: transactionId, summary: `Komisyon kaydı oluşturuldu: ${input.transactionNo.trim()} · ${participantRows.length} paydaş · danışman net ${consultantShare.toFixed(2)} · Global ofis ${global1881Share.toFixed(2)} · dış ofis ${externalOfficeShare.toFixed(2)}${input.externalOfficeAgreementReference?.trim() ? ` · dış ofis anlaşması ${input.externalOfficeAgreementReference.trim()}` : ""}${input.externalOfficeAgreementPartyName?.trim() ? ` · taraf ${input.externalOfficeAgreementPartyName.trim()}` : ""}${input.externalOfficeAgreementMinFee || input.externalOfficeAgreementMaxFee ? ` · kapsam ${input.externalOfficeAgreementMinFee ?? "-"}-${input.externalOfficeAgreementMaxFee ?? "-"}` : ""}` });
...
export async function createContractFormTemplate(input: {
  ...
  for (const field of getDefaultFormFields(input.formType)) {
    await db.insert(contractFormFields).values({
      ...
      optionsJson: "optionsJson" in field && typeof field.optionsJson === "string" ? field.optionsJson : null,
  ...
  if (input.formType === "land_share") {
    await db.insert(contractFormClauses).values(
      LAND_SHARE_FIXED_CLAUSES.map(clause => ({
        ...
        title: clause.title,
        bodyTemplate: input.formType === "land_share" ? parameterizeLandShareClauseBody(clause.bodyTemplate) : clause.bodyTemplate,
