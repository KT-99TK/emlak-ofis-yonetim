...
- [ ] Kira sözleşmesi ve Tahliye/Teslim Etme/Teslim Alma/Demirbaş eklerinin ortak başlığındaki `Kira sözleşmesi kaydı: Kayıtta atanacak` taslak izini müşteri yüzeyinden tamamen kaldır; eklerde taslak ve kayıtlı durumda iç kayıt numarası/iz başlığı görünmesin, yalnız ana kira belgesinde gerçek kayıt sonrası küçük/silik danışman-form izi korunabilsin; kullanıcı Windows ekranında hem Teslim Etme hem Tahliye Taahhütnamesinde metnin göründüğünü kanıtladı
...
- [ ] Aktif sayfa düzeni kabulünde sol menüde `Kasa ve Banka` öğesini `Sözleşme ve Finansal İstatistikler` öğesinin hemen altına taşı; rota/rol erişimini değiştirme, `Yedekleri Birleştir` bunun altında kalsın ve sonraki toplu Windows paketinde kullanıcıyla doğrula; kullanıcının 1.0.17 ekranında eski sıra görüldüğü için henüz kabul edilmedi
