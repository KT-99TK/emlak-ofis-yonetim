...
export default function ContractFormTemplates() {
  ...
  return <div className="min-h-screen bg-[#f7f7f4] px-5 py-7 md:px-10 md:py-9">
    ...
    {selectedId !== null && <Card className="mt-6 rounded-2xl border-[#e5e8e3] bg-white/80"><CardHeader><div className="flex flex-wrap items-start justify-between gap-3"><div><CardTitle className="font-serif text-xl">{bundle.data?.template.title ?? "Şablon ayrıntıları"}</CardTitle><p className="text-xs text-[#87938f]">Bölümler ve alanlar örnek metin gelmeden boş tutulur. Ek maddeler yayınlanmadan önce manager kontrolünde kalır.</p><p className="mt-2 text-xs font-semibold text-[#4b8878]">Çıktıya aday aktif ek madde: {preview.data?.clauses.length ?? 0}</p></div><div className="flex gap-2">{bundle.data?.template.status === "draft" && <Button size="sm" variant="outline" onClick={() => setTemplateStatus.mutate({ templateId: selectedId, status: "review" })}>İncelemeye al</Button>}{bundle.data?.template.status === "review" && <Button size="sm" variant="outline" onClick={() => setTemplateStatus.mutate({ templateId: selectedId, status: "published" })}>Yayınla</Button>}{bundle.data?.template.status === "published" && <Button size="sm" variant="outline" onClick={() => setTemplateStatus.mutate({ templateId: selectedId, status: "archived" })}>Arşivle</Button>}</div></div></CardHeader><CardContent className="grid gap-6 lg:grid-cols-[.9fr_1.1fr]">
