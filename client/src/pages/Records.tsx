...
export default function Records() {
  ...
  const printRecordsPdf = () => {
    ...
    setPrintPreviewHtml(`<h1>GLOBAL 1881 — ${safe(config.title).toUpperCase()}</h1><p>Filtreler: ${safe(filterSummary)} · Kayıt sayısı: ${visibleItems.length}</p><table><thead><tr>${headings}</tr></thead><tbody>${rows || '<tr><td colspan="6">Filtreye uyan kayıt bulunamadı.</td></tr>'}</tbody></table>`);
  };
