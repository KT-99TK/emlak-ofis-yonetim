...
import { Banknote, Eye, EyeOff, FolderKanban, Plus, Printer, RefreshCw, ShieldCheck, UserRound } from "lucide-react";
...
export default function Records() {
  ...
  const printRecordsPdf = () => {
    ...
    const rows = visibleItems.map((item: any) => `<tr><td>${safe(item.referenceNo || item.id)}</td><td>${safe(item.name || item.title || item.description)}</td><td>${safe(item.consultantCode)}</td><td>${safe(item.status || "Aktif")}</td><td>${safe(item.amount)}</td><td>${safe(item.address || item.entryType || item.listingType)}</td></tr>`).join("");
    ...
    popup.document.write(`<!doctype html><html lang="tr"><head><meta charset="utf-8"><title>Global 1881 — ${safe(config.title)} PDF</title><style>@page{size:A4 landscape;margin:12mm}body{font-family:Arial,sans-serif;color:#24322f;font-size:10px}h1{font-size:18px;margin-bottom:5px}p{color:#64736e}table{width:100%;border-collapse:collapse}th,td{border:1px solid #b8c5bf;padding:5px;text-align:left}th{background:#eaf1ed}</style></head><body><h1>GLOBAL 1881 — ${safe(config.title).toUpperCase()}</h1><p>Filtreler: ${safe(filterSummary)} · Kayıt sayısı: ${visibleItems.length}</p><table><thead><tr><th>Referans / ID</th><th>Kayıt</th><th>Danışman</th><th>Durum</th><th>Tutar</th><th>Detay</th></tr></thead><tbody>${rows || '<tr><td colspan="6">Filtreye uyan kayıt bulunamadı.</td></tr>'}</tbody></table></body></html>`);
