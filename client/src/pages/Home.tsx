  ...
  return (
    <div className="min-h-screen bg-[#f7f7f4] text-[#1d2928]">
      <div className="mx-auto max-w-[1440px] px-5 py-7 md:px-10 md:py-9">
        ...
        {!obligationsQuery.isLoading && !obligationsQuery.isError && dueObligations.length > 0 && <div className="mb-4 rounded-xl border border-[#ead6d0] bg-[#fff8f6] px-4 py-3 text-xs text-[#a85745]" role="alert"><strong>Vade uyarısı:</strong> {dueObligations.map((item) => `${item.title} · ${new Date(item.dueDate).toLocaleDateString("tr-TR")}`).join("  |  ")} — tarih yaklaşan kira/vergi işlemlerini kontrol edin.</div>}
        ...
        <section className="grid gap-6 xl:grid-cols-[1.35fr_.65fr]">
          ...
          <Card className="rounded-2xl border-[#e5e8e3] bg-white/80 shadow-[0_10px_30px_rgba(26,46,42,.04)]"><CardHeader className="flex flex-row items-center justify-between p-6 pb-2"><div><CardTitle className="font-serif text-xl font-medium">Yaklaşan işler</CardTitle><p className="mt-1 text-xs text-[#85918d]">Bugün ve önümüzdeki 7 gün</p></div><CalendarClock className="h-5 w-5 text-[#a17b43]" /></CardHeader><CardContent className="p-6 pt-4"><div className="space-y-4"><div className="flex gap-3 border-b border-[#edf0ec] pb-4"><span className="mt-1 h-2 w-2 rounded-full bg-[#bd975d]" /><div><p className="text-sm font-semibold text-[#34433f]">3 sözleşme onayı</p><p className="mt-1 text-xs text-[#87938f]">Manager incelemesi bekliyor</p></div></div><div className="flex gap-3 border-b border-[#edf0ec] pb-4"><span className="mt-1 h-2 w-2 rounded-full bg-[#2b786e]" /><div><p className="text-sm font-semibold text-[#34433f]">2 tahsilat vadesi</p><p className="mt-1 text-xs text-[#87938f]">Toplam ₺ 76.000</p></div></div><div className="flex gap-3"><span className="mt-1 h-2 w-2 rounded-full bg-[#a85745]" /><div><p className="text-sm font-semibold text-[#34433f]">1 portföy belgesi eksik</p><p className="mt-1 text-xs text-[#87938f]">Yetki sözleşmesi tamamlanmalı</p></div></div></div></CardContent></Card>
        </section>
