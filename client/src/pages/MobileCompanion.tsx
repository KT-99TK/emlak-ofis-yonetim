...
export default function MobileCompanion() {
  ...
  const onlineStart = trpc.onlineStart.status.useQuery(undefined, { enabled: isAuthenticated, refetchOnWindowFocus: true, retry: false });
  ...
  const onlineStartPending = !onlineStart.data || new Date() < new Date(onlineStart.data.effectiveAt);
  ...
  const refresh = () => {
    void Promise.all([summary.refetch(), onlineStart.refetch(), clients.refetch(), properties.refetch(), contracts.refetch(), obligations.refetch(), documents.refetch(), archiveDocuments.refetch(), treasury.refetch()]);
  };

  const handlePdf = (file?: File) => {
    setNotice(null);
    if (centralOperationsLocked) {
      setNotice("Merkezi online başlangıç tarihi gelmeden yeni imzalı PDF eklenemez.");
      return;
    }
  ...
  const submitCashMovement = () => {
    if (centralOperationsLocked) { setNotice("Merkezi online başlangıç tarihi gelmeden kasa hareketi beyan edilemez."); return; }
  ...
  const submitCashCount = () => {
    if (centralOperationsLocked) { setNotice("Merkezi online başlangıç tarihi gelmeden gün sonu kasa sayımı kapatılamaz."); return; }
  ...
  return <main className="min-h-screen bg-[#f7f7f4] pb-24 text-[#243733]">
    ...
    <section className="mx-auto max-w-xl space-y-4 px-4 pt-5">
      {!onlineStart.isLoading && onlineStartPending && <section className="rounded-2xl border border-[#e7d9b7] bg-[#fffaf0] p-4 text-xs text-[#725d36]" role="status"><p className="font-semibold">Merkezi online başlangıç bekliyor.</p><p className="mt-1 leading-5">Eski offline sözleşme, arşiv ve finans kayıtları taşınmaz. Yeni merkezi kayıtlar, broker manager geçiş tarihini ayarladığında sıfırdan başlayacaktır.</p>{isManager && <p className="mt-2 font-semibold">Geçiş tarihini web yönetimindeki Online Başlangıç menüsünden ayarlayabilirsiniz.</p>}</section>}
      {section === "gundem" && <>
        ...
        {treasury.data && <section className="rounded-2xl border border-[#d9c99e] bg-[#fffdf7] p-4 shadow-sm">
          ...
          <details className="mt-3 rounded-xl border border-[#ded6bb] bg-white p-3"><summary className="cursor-pointer text-sm font-semibold text-[#38584c]">Kasa hareketi beyan et</summary><p className="mt-2 text-xs leading-5 text-[#6f7d76]">Fatura veya makbuz numarası zorunludur. Beyan, broker manager doğrulayana kadar bakiyeye girmez.</p><div className="mt-3 grid gap-2"><select disabled={centralOperationsLocked} value={cashDraft.movementType} onChange={(event) => setCashDraft({ ...cashDraft, movementType: event.target.value as typeof cashDraft.movementType })} className="h-10 rounded-lg border border-[#d9dfd8] bg-white px-2 text-xs disabled:cursor-not-allowed disabled:opacity-60"><option value="bankToCash">Bankadan ofis kasasına çekim</option><option value="cashExpense">Kasadan nakit ödeme</option><option value="cashReceipt">Kasaya nakit tahsilat</option><option value="cashDeposit">Kasadan bankaya yatırma</option><option value="other">Diğer kasa hareketi</option></select><input disabled={centralOperationsLocked} inputMode="decimal" value={cashDraft.amount} onChange={(event) => setCashDraft({ ...cashDraft, amount: event.target.value })} placeholder="Tutar (TL)" className="h-10 rounded-lg border border-[#d9dfd8] px-2 text-xs disabled:cursor-not-allowed disabled:opacity-60" /><input disabled={centralOperationsLocked} value={cashDraft.counterparty} onChange={(event) => setCashDraft({ ...cashDraft, counterparty: event.target.value })} placeholder="Ödeme yapılan / tahsil eden kişi veya firma" className="h-10 rounded-lg border border-[#d9dfd8] px-2 text-xs disabled:cursor-not-allowed disabled:opacity-60" /><input disabled={centralOperationsLocked} value={cashDraft.evidenceReference} onChange={(event) => setCashDraft({ ...cashDraft, evidenceReference: event.target.value })} placeholder="Fatura, makbuz veya banka dekont no *" className="h-10 rounded-lg border border-[#d9dfd8] px-2 text-xs disabled:cursor-not-allowed disabled:opacity-60" /><textarea disabled={centralOperationsLocked} value={cashDraft.note} onChange={(event) => setCashDraft({ ...cashDraft, note: event.target.value })} placeholder="Kısa açıklama (opsiyonel)" className="min-h-16 rounded-lg border border-[#d9dfd8] p-2 text-xs disabled:cursor-not-allowed disabled:opacity-60" /><button type="button" disabled={centralOperationsLocked || declareCashMovement.isPending || !cashDraft.amount || !cashDraft.counterparty || !cashDraft.evidenceReference} onClick={submitCashMovement} className="rounded-lg bg-[#173e39] px-3 py-2 text-xs font-semibold text-white disabled:bg-[#93aaa1]">{declareCashMovement.isPending ? "Kaydediliyor…" : "Kanıtla birlikte beyan et"}</button></div></details>
      ...
      {section === "belgeler" && <>
        ...
        {isManager && !legacyArchiveImportEnabled && <section className="rounded-2xl border border-[#e7d9b7] bg-[#fffaf0] p-4 text-[#725d36]"><div className="flex items-center gap-2"><ShieldCheck className="h-4 w-4 text-[#a17b43]" /><h2 className="font-serif text-xl">Geçmiş Offline Arşiv</h2></div><p className="mt-2 text-xs leading-5">Temiz online başlangıçta eski offline PDF, sözleşme ve finans kayıtları merkezi sisteme yüklenmez. Eski dosyalar yerel geçmiş arşivinde korunur; yalnız geçişten sonra merkezi olarak açılan yeni sözleşmelere imzalı PDF eklenebilir.</p></section>}
        {isManager && legacyArchiveImportEnabled && <section className="rounded-2xl border border-[#dce7df] bg-white p-4">
