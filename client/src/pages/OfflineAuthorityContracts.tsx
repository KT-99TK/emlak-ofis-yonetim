...
export default function OfflineAuthorityContracts() {
  ...
  const propertyBelongsToVisibleClient = (record: OfflineRecord) => {
    ...
    const clientId = record.details.match(/(?:^|\|)\s*MÜŞTERİ_ID:\s*([^|]+)/i)?.[1]?.trim();
    const ownerName = record.details.match(/(?:^|\|)\s*MALİK:\s*([^|]+)/i)?.[1]?.trim();
  ...
  const selectedPropertyRecords = useMemo(() => {
    if (!clientRecordId) return propertyRecords;
    const linked = propertyRecords.filter((record) => record.details.match(/(?:^|\|)\s*MÜŞTERİ_ID:\s*([^|]+)/i)?.[1]?.trim() === clientRecordId);
    return linked.length ? linked : propertyRecords;
  }, [clientRecordId, propertyRecords]);
  ...
  const chooseProperty = (id: string) => { setSaveVisualState("idle"); setPropertyRecordId(id); const record = selectedPropertyRecords.find((item) => item.id === id); if (record) setDetails((current) => normalizeAuthorityDetails({ ...current, propertyAddress: record.details ? `${record.title} · ${record.details}` : record.title })); };
