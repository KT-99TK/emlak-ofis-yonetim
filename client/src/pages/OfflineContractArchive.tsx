...
import { compareContractArchiveChronologically, contractArchiveDocumentTypes, createContractArchiveMetadata, formatArchiveByteSize, parseContractArchiveMetadata, type ContractArchiveDocumentType } from "@/lib/contractArchive";
...
export default function OfflineContractArchive() {
  ...
  const visibleRows = useMemo(() => records
    ...
    .filter(({ record, metadata }) => [record.title, record.userId, metadata.customerName, metadata.historicalActivity, metadata.originalFileName, metadata.documentTypeLabel].join(" ").toLocaleLowerCase("tr-TR").includes(query.trim().toLocaleLowerCase("tr-TR"))), [records, accessContext.userId, accessContext.role, accessContext.managerSessionActive, query]);

  const customerGroups = useMemo(() => {
    const groups = new Map<string, typeof visibleRows>();
    visibleRows.forEach((row) => {
      const name = row.metadata.customerName.trim() || "Müşteri adı belirtilmemiş";
  ...
  const importPdf = async () => {
    ...
    try {
      ...
      for (const file of files) {
        const metadata = createContractArchiveMetadata({ documentType, customerName, documentDate, historicalActivity, originalFileName: file.originalName, sha256: file.sha256, byteSize: file.byteSize, storageKey: file.archiveKey, archiveNote });
      ...
      setCustomerName(""); setDocumentDate(""); setHistoricalActivity(""); setArchiveNote(""); await refresh();
  ...
  return <div className="min-h-screen bg-[#f7f7f4] px-5 py-7 md:px-10 md:py-9">
    <header className="mb-7 flex flex-wrap items-end justify-between gap-4">
      <div>
        ...
        <p className="mt-2 max-w-3xl text-sm text-[#70807c]">Eski PDF belgeleri ilgili danışman ve <strong className="font-semibold text-[#34433f]">müşteri</strong> adına bağlanır. Her müşterinin geçmiş işlemleri aynı arşivde görünür; bu belgeler aktif sözleşme, vade, tahsilat, ön muhasebe veya performans kaydı oluşturmaz.</p>
      </div>
    ...
    <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_290px]">
      <div className="space-y-5">
        {canImport ? <Card className="rounded-2xl border-[#e5e8e3] bg-white"><CardHeader><CardTitle className="flex items-center gap-2 font-serif text-xl text-[#223230]"><Upload className="h-5 w-5 text-[#a17b43]" /> Eski PDF içe aktar</CardTitle><p className="text-xs leading-5 text-[#70807c]">Dosya baytları Windows kullanıcı verisi altındaki arşiv klasörüne alınır; şifreli JSON yedeklerinde yalnız metadata bulunur. Birleştirme sonrası PDF dosyası ayrıca güvenli taşınmalıdır.</p></CardHeader><CardContent className="grid gap-4 md:grid-cols-2">
          ...
          <div><Label htmlFor="archive-customer-name">Müşteri adı</Label><Input id="archive-customer-name" className="mt-1.5 bg-white" value={customerName} onChange={(event) => setCustomerName(event.target.value)} placeholder="Ör. Ayşe Demir" /></div>
