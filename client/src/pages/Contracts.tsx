      ...
      <Card className="rounded-2xl border-[#dfe8e1] bg-white shadow-[0_12px_28px_rgba(23,62,57,.06)]">
        ...
        <CardContent className="space-y-3 px-5 py-4">
          <div className="grid gap-3 sm:grid-cols-2">
            ...
            <div><label className="mb-1 block text-[11px] font-semibold text-[#56635f]">Müşteri kütüğü</label><Select value={selectedClientId} onValueChange={setSelectedClientId}><SelectTrigger className="h-10 bg-white"><SelectValue placeholder={clients.isLoading ? "Müşteriler yükleniyor…" : "Müşteri no / ad seçin"} /></SelectTrigger><SelectContent>{(clients.data ?? []).map((client) => <SelectItem key={client.id} value={String(client.id)}>{client.referenceNo ? `${client.referenceNo} · ` : ""}{client.name}{client.consultantCode ? ` · ${client.consultantCode}` : ""}</SelectItem>)}</SelectContent></Select><p className="mt-1 text-[10px] text-[#8b9691]">Seçilen müşteri sözleşme kaydına bağlanır; müşteri verisi kütükten çağrılır.</p></div>
          </div>
