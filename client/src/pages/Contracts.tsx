...
export default function Contracts() {
  ...
  const printFilteredContractsPdf = () => {
    ...
    popup.document.write(`<!doctype html><html lang="tr"><head><meta charset="utf-8"><title>Global 1881 — Sözleşmeler PDF</title><style>@page{size:A4 landscape;margin:12mm}body{font-family:Arial,sans-serif;color:#24322f;font-size:10px}h1{font-size:18px}p{color:#64736e}table{width:100%;border-collapse:collapse}th,td{border:1px solid #b8c5bf;padding:5px;text-align:left}th{background:#eaf1ed}</style></head><body><h1>GLOBAL 1881 — SÖZLEŞME KAYITLARI</h1><p>Filtreler: ${safe(filterSummary)} · Kayıt sayısı: ${visibleContracts.length}</p><table><thead><tr><th>Sözleşme No</th><th>Başlık</th><th>Tür</th><th>Durum</th><th>Bedel</th></tr></thead><tbody>${rows || '<tr><td colspan="5">Filtreye uyan sözleşme bulunamadı.</td></tr>'}</tbody></table></body></html>`);
