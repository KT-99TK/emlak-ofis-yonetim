...
export function createTransactionFromContract(record: OfflineRecord, allRecords: OfflineRecord[], actor: string): OfflineTransactionDetails | null {
  ...
  if (snapshot.schema?.startsWith("global1881-offline-authority") && (snapshot as AuthoritySnapshot).mode === "sale") {
    ...
    const details: AuthorityContractDetails = {
      mode: "sale", ownerName: authority.ownerName ?? "", ownerIdentity: authority.ownerIdentity ?? "", ownerPhone: authority.ownerPhone ?? "", ownerAddress: authority.ownerAddress ?? "", eidsAuthorizationNumber: authority.eidsAuthorizationNumber ?? "", eidsAuthorizedAt: authority.eidsAuthorizedAt ?? "", eidsAuthorizedBy: authority.eidsAuthorizedBy ?? "", propertyNeighborhood: authority.propertyNeighborhood ?? "", propertyAddress: authority.propertyAddress ?? "", parcelInfo: authority.parcelInfo ?? "", propertyType: authority.propertyType ?? "", grossM2: authority.grossM2 ?? "", roomCount: authority.roomCount ?? "", floorAndView: authority.floorAndView ?? "", condition: authority.condition ?? "", price: authority.price ?? "", currency: authority.currency === "USD" || authority.currency === "EUR" ? authority.currency : "TRY", serviceFeeRate: authority.serviceFeeRate ?? "", serviceFeeAmount: authority.serviceFeeAmount ?? "", vatCollection: authority.vatCollection === "included" ? "included" : "separate", contractDate: authority.contractDate ?? today(), authorityDurationMonths: authority.authorityDurationMonths ?? "3", consultantName: authority.consultantName ?? "", consultantPhone: authority.consultantPhone ?? "", consultantCode: authority.consultantCode ?? "", consultantTitle: authority.consultantTitle ?? "", officeName: authority.officeName ?? "", officeAuthorizationNo: authority.officeAuthorizationNo ?? "", officeTaxOffice: authority.officeTaxOffice ?? "", officeTaxNo: authority.officeTaxNo ?? "", officePhone: authority.officePhone ?? "", officeAddress: authority.officeAddress ?? "",
    };
  ...
  if (snapshot.schema?.startsWith("global1881-offline-rental")) {
    ...
    const details: OfflineRentalDetails = { useType: rental.useType === "commercial" ? "commercial" : "residential", ownerName: rental.ownerName ?? "", ownerIdentity: rental.ownerIdentity ?? "", ownerPhone: rental.ownerPhone ?? "", ownerEmail: rental.ownerEmail ?? "", ownerAddress: rental.ownerAddress ?? "", ownerVatRegistered: Boolean(rental.ownerVatRegistered), tenantName: rental.tenantName ?? "", tenantIdentity: rental.tenantIdentity ?? "", tenantPhone: rental.tenantPhone ?? "", tenantEmail: rental.tenantEmail ?? "", tenantTaxOffice: rental.tenantTaxOffice ?? "", tenantWithholdingRegistered: Boolean(rental.tenantWithholdingRegistered), tenantAddress: rental.tenantAddress ?? "", guarantorName: rental.guarantorName ?? "", guarantorIdentity: rental.guarantorIdentity ?? "", guarantorLimit: rental.guarantorLimit ?? "", hasGuarantor: Boolean(rental.hasGuarantor), signedByParties: Boolean(rental.signedByParties), signedAt: rental.signedAt ?? "", propertyNeighborhood: rental.propertyNeighborhood ?? "", propertyAddress: rental.propertyAddress ?? "", propertyType: rental.propertyType ?? "", parcelInfo: rental.parcelInfo ?? "", independentSectionNo: rental.independentSectionNo ?? "", occupancyPermit: rental.occupancyPermit === "present" || rental.occupancyPermit === "absent" ? rental.occupancyPermit : "unknown", condominiumStatus: rental.condominiumStatus === "yes" || rental.condominiumStatus === "no" ? rental.condominiumStatus : "unknown", fixtures: rental.fixtures ?? "", electricityMeterNo: rental.electricityMeterNo ?? "", waterMeterNo: rental.waterMeterNo ?? "", naturalGasMeterNo: rental.naturalGasMeterNo ?? "", daskPolicyNo: rental.daskPolicyNo ?? "", meterNotes: rental.meterNotes ?? "", monthlyRent: rental.monthlyRent ?? "", deposit: rental.deposit ?? "", proratedStartDate: rental.proratedStartDate ?? "", proratedEndDate: rental.proratedEndDate ?? "", proratedDays: rental.proratedDays ?? "", proratedAmount: rental.proratedAmount ?? "", currency: "TRY", vatCollection: "separate", paymentDay: rental.paymentDay ?? "1", firstPaymentDueDate: rental.firstPaymentDueDate ?? rental.startDate ?? today(), appendixSelection: { evacuation: Boolean(rental.appendixSelection?.evacuation), handover: rental.appendixSelection?.handover !== false, return: Boolean(rental.appendixSelection?.return), fixtures: rental.appendixSelection?.fixtures !== false }, iban: rental.iban ?? "", startDate: rental.startDate ?? today(), durationMonths: rental.durationMonths ?? "12", noticeDays: rental.noticeDays ?? "60", kdvIncluded: Boolean(rental.kdvIncluded), usagePurpose: rental.usagePurpose ?? "", residentsCount: rental.residentsCount ?? "", courtCity: rental.courtCity ?? "", documentPlace: rental.documentPlace ?? "", ownerApproval: rental.ownerApproval === "approved" ? "approved" : "pending", consultantName: rental.consultantName ?? "", consultantCode: rental.consultantCode ?? "", officeName: rental.officeName ?? "", officeAuthorizationNo: rental.officeAuthorizationNo ?? "" };
