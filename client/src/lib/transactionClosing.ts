...
export type TransactionCollection = { id: string; category: CollectionCategory; label: string; expectedAmount: number; collectedAmount: number; currency: "TRY" | "USD" | "EUR"; dueDate?: string; collectedAt?: string; method?: PaymentMethod; reference?: string; state: CollectionState; note?: string };
...
function collection(category: CollectionCategory, expectedAmount: number, currency: "TRY" | "USD" | "EUR", dueDate?: string): TransactionCollection {
  return { id: `${category}-${crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(36).slice(2)}`}`, category, label: collectionCategoryLabels[category], expectedAmount: Math.max(0, Math.round(expectedAmount)), collectedAmount: 0, currency, dueDate: asDate(dueDate), state: "planned" };
}
...
export function createTransactionFromContract(record: OfflineRecord, allRecords: OfflineRecord[], actor: string): OfflineTransactionDetails | null {
  ...
  if (snapshot.schema?.startsWith("global1881-offline-authority") && (snapshot as AuthoritySnapshot).mode === "sale") {
    ...
    const collections = [collection("serviceFee", fee, details.currency, details.contractDate), ...(details.vatCollection === "separate" ? [collection("vat", Math.round(fee * 0.2), details.currency, details.contractDate)] : [])].filter((item) => item.expectedAmount > 0);
  ...
  if (snapshot.schema?.startsWith("global1881-offline-rental")) {
    ...
    const details: OfflineRentalDetails = { useType: rental.useType === "commercial" ? "commercial" : "residential", ownerName: rental.ownerName ?? "", ownerIdentity: rental.ownerIdentity ?? "", ownerPhone: rental.ownerPhone ?? "", ownerAddress: rental.ownerAddress ?? "", tenantName: rental.tenantName ?? "", tenantIdentity: rental.tenantIdentity ?? "", tenantPhone: rental.tenantPhone ?? "", tenantAddress: rental.tenantAddress ?? "", guarantorName: rental.guarantorName ?? "", guarantorIdentity: rental.guarantorIdentity ?? "", guarantorLimit: rental.guarantorLimit ?? "", hasGuarantor: Boolean(rental.hasGuarantor), propertyNeighborhood: rental.propertyNeighborhood ?? "", propertyAddress: rental.propertyAddress ?? "", propertyType: rental.propertyType ?? "", parcelInfo: rental.parcelInfo ?? "", fixtures: rental.fixtures ?? "", meterNotes: rental.meterNotes ?? "", monthlyRent: rental.monthlyRent ?? "", deposit: rental.deposit ?? "", currency: "TRY", vatCollection: rental.vatCollection === "included" ? "included" : "separate", paymentDay: rental.paymentDay ?? "1", iban: rental.iban ?? "", startDate: rental.startDate ?? today(), durationMonths: rental.durationMonths ?? "12", noticeDays: rental.noticeDays ?? "60", kdvIncluded: Boolean(rental.kdvIncluded), usagePurpose: rental.usagePurpose ?? "", residentsCount: rental.residentsCount ?? "", courtCity: rental.courtCity ?? "", documentPlace: rental.documentPlace ?? "", ownerApproval: rental.ownerApproval === "approved" ? "approved" : "pending", consultantName: rental.consultantName ?? "", consultantCode: rental.consultantCode ?? "", officeName: rental.officeName ?? "", officeAuthorizationNo: rental.officeAuthorizationNo ?? "" };
    const summary = calculateRentalSummary(details);
    const deposit = toAmount(details.deposit);
    const collections = [collection("deposit", deposit, "TRY", details.startDate), collection("rentalFirstMonth", summary.monthlyRent, "TRY", summary.firstDueDate), collection("serviceFee", summary.monthlyRent, "TRY", details.startDate), ...(details.vatCollection === "separate" ? [collection("vat", Math.round(summary.monthlyRent * 0.2), "TRY", details.startDate)] : [])].filter((item) => item.expectedAmount > 0);
