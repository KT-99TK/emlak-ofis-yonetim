...
function safeAuthorityDetails(snapshot: AuthoritySnapshot): AuthorityContractDetails {
  return {
    mode: snapshot.mode === "sale" ? "sale" : "rent", ownerName: snapshot.ownerName ?? "", ownerIdentity: snapshot.ownerIdentity ?? "", ownerPhone: snapshot.ownerPhone ?? "", ownerAddress: snapshot.ownerAddress ?? "", eidsAuthorizationNumber: snapshot.eidsAuthorizationNumber ?? "", eidsAuthorizedAt: snapshot.eidsAuthorizedAt ?? "", eidsAuthorizedBy: snapshot.eidsAuthorizedBy ?? "",
