...
export function ExchangeRateCard() {
  const rateQuery = trpc.exchangeRates.eurTry.useQuery(undefined, {
    retry: false,
        ...
        {rateQuery.isLoading ? (
          ...
          <>
            <div className="flex items-end justify-between gap-3">
              ...
              <span className="pb-1 text-xs font-medium text-[#70807c]">₺ / EUR</span>
            </div>
            <p className="mt-2 text-[11px] text-[#87938f]">
              Veri tarihi: {formatTurkishDate(rate.rateDate)} · Kaynak: ECB
            </p>
            <p className="mt-3 border-t border-[#edf0ec] pt-3 text-[10px] leading-4 text-[#9aa6a1]">
              Bilgilendirme amaçlı referans kurudur; muhasebe veya işlem kuru olarak kullanılmaz.
            </p>
          </>
        )}
      </CardContent>
