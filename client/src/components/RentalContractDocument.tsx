...
export default function RentalContractDocument({ details, contractNo, fontSize }: RentalContractDocumentProps) {
  ...
  return <article className="authority-print-document authority-contract-document bg-[#fff] text-[#1c2524]" style={{ "--authority-print-font-size": `${fontSize}pt` } as React.CSSProperties}>
    <header className="authority-document-brand"><div className="authority-document-seal">1881</div><div><p className="authority-document-office-name">{value(details.officeName)}</p><p>Yetki Belgesi No: {value(details.officeAuthorizationNo)}</p><p>Offline kira sözleşmesi · Kayıt No: {value(contractNo)}</p></div></header>
    ...
    <section className="authority-document-signatures"><div><p>KİRAYA VEREN</p><strong>{value(details.ownerName)}</strong><span>İmza</span></div><div><p>KİRACI</p><strong>{value(details.tenantName)}</strong><span>İmza</span></div><div><p>EMLAK DANIŞMANI</p><strong>{value(details.consultantName)}</strong><span>İmza</span></div></section>
    <footer className="authority-print-running-footer">Global 1881 Gayrimenkul · Kira sözleşmesi taslağı · Kayıt: {value(contractNo)}</footer>
  </article>;
}
