...
import { ArrowUpRight, BellRing, CalendarClock, CheckCircle2, ShieldCheck } from "lucide-react";
...
export type OfficeFlowContract = { id: number | string; status: string; ownerApprovalStatus?: string | null };
export type OfficeFlowLedgerEntry = { id: number | string; entryType: string; status: string };
...
export function OfficeFlowPanel({ role, obligations, contracts = [], ledgerEntries = [], onOpenObligations, onOpenContracts, onOpenAccounting, now = new Date() }: OfficeFlowPanelProps) {
  ...
  const weekCount = rankedItems.filter((item) => item.days >= 0 && item.days <= 7).length;
  const visiblePersonalItems = rankedItems.slice(0, 3);
  const reviewContractCount = contracts.filter((item) => item.status === "draft" || item.status === "review" || item.ownerApprovalStatus === "pending").length;
  const openCollectionCount = ledgerEntries.filter((item) => item.entryType === "receivable" && item.status !== "paid" && item.status !== "cancelled").length;
  ...
  return (
    <Card className="relative overflow-hidden rounded-2xl border-[#0f5145] bg-[#0f473f] text-white shadow-[0_18px_42px_rgba(15,71,63,.22)]">
      ...
      <CardContent className="relative p-6">
        ...
        {isManager ? (
          <div className="mt-6 space-y-2" aria-label="Ofis geneli istisna özeti">
            <div className="rounded-xl border border-white/10 bg-white/[.08] px-3 py-3"><div className="flex items-center justify-between gap-3"><span className="text-xs font-medium text-[#d8e8e3]">Gecikmiş vade</span><strong className="font-serif text-2xl text-[#ffb099]">{overdueCount}</strong></div><p className="mt-1 text-[11px] text-[#aac8c0]">İnceleme gerektiren kayıt sayısı</p></div>
            <div className="rounded-xl border border-white/10 bg-white/[.08] px-3 py-3"><div className="flex items-center justify-between gap-3"><span className="text-xs font-medium text-[#d8e8e3]">Önümüzdeki 7 gün</span><strong className="font-serif text-2xl text-[#e6c47d]">{weekCount}</strong></div><p className="mt-1 text-[11px] text-[#aac8c0]">Danışmanların kendi iş listelerinde görünür</p></div>
        ...
        {!isManager && <div className="mt-3 grid grid-cols-2 gap-2"><button type="button" onClick={onOpenContracts} className="rounded-xl border border-white/10 bg-white/[.06] px-3 py-2 text-left transition-colors hover:bg-white/[.12]"><span className="block text-lg font-serif text-white">{reviewContractCount}</span><span className="block text-[10px] leading-4 text-[#b4d1ca]">Sözleşme adımı</span></button><button type="button" onClick={onOpenAccounting} className="rounded-xl border border-white/10 bg-white/[.06] px-3 py-2 text-left transition-colors hover:bg-white/[.12]"><span className="block text-lg font-serif text-white">{openCollectionCount}</span><span className="block text-[10px] leading-4 text-[#b4d1ca]">Açık tahsilat</span></button></div>}

        <div className="mt-5 flex items-center justify-between gap-3 border-t border-white/10 pt-4"><span className="flex items-center gap-1.5 text-[10px] text-[#b4d1ca]"><ShieldCheck className="h-3.5 w-3.5" /> Rolünüze göre filtrelendi</span><Button variant="ghost" onClick={onOpenObligations} className="h-8 rounded-lg px-2 text-xs font-semibold text-[#e6c47d] hover:bg-white/10 hover:text-white">Vadeleri aç <ArrowUpRight className="ml-1 h-3.5 w-3.5" /></Button></div>
      </CardContent>
      <Dialog open={showManagerExceptions} onOpenChange={setShowManagerExceptions}><DialogContent className="max-w-sm border-[#a8ccc2] bg-[#f7fbf9]"><DialogHeader><DialogTitle className="font-serif text-2xl text-[#173e39]">İstisna özeti</DialogTitle><DialogDescription>Bu görünüm danışman, müşteri ve sözleşme adı açmadan ofis geneli takip gereksinimini gösterir.</DialogDescription></DialogHeader><div className="grid gap-2 text-sm"><div className="rounded-xl border border-[#dce8e4] bg-white p-3"><strong className="text-[#24413b]">{reviewContractCount} sözleşme işlemi</strong><p className="mt-1 text-xs text-[#6a7e77]">Taslak, inceleme veya malik onayı bekleyen kayıtlar.</p></div><div className="rounded-xl border border-[#dce8e4] bg-white p-3"><strong className="text-[#24413b]">{openCollectionCount} açık tahsilat</strong><p className="mt-1 text-xs text-[#6a7e77]">Kapanmamış alacak kaydı sayısı.</p></div><div className="rounded-xl border border-[#dce8e4] bg-white p-3"><strong className="text-[#24413b]">{overdueCount} gecikmiş vade</strong><p className="mt-1 text-xs text-[#6a7e77]">Danışman iş listelerinde önceliklendirilen kayıtlar.</p></div></div><div className="flex justify-end gap-2"><Button variant="outline" size="sm" onClick={() => setShowManagerExceptions(false)}>Kapat</Button><Button size="sm" onClick={() => { setShowManagerExceptions(false); onOpenContracts?.(); }}>Sözleşmeleri aç</Button></div></DialogContent></Dialog>
    </Card>
  );
}
