...
export default function MultiPropertyIntakeForm({ records, access, onRefresh }: MultiPropertyIntakeFormProps) {
  ...
  return <Card className="mb-6 rounded-2xl border-[#dbe5dd] bg-white/90">
    ...
    <CardContent className="space-y-4">
      ...
      <div className="flex flex-wrap gap-2"><Button type="button" variant="outline" onClick={addRow}><Plus className="mr-2 h-4 w-4" /> Mülk satırı ekle</Button><Button type="button" onClick={() => void save()} disabled={saving} className="bg-[#173e39] hover:bg-[#20554e]"><Save className="mr-2 h-4 w-4" />{saving ? "Kaydediliyor…" : "Mülkleri toplu kaydet"}</Button></div>
    </CardContent>
  </Card>;
}
